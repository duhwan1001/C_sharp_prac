...
class program
{
    ...
    private static void serverFunc(object obj)
    {
        ...
        {
            IPAddress ipAddress = IPAddress.Parse("0.0.0.0"); // 모든 IP에 바인딩 할수 있도록 특별한 주소("0.0.0.0")을 제공한다.
            IPEndPoint endPoint = new IPEndPoint(ipAddress, 10200); // 10200 : 포트 번호

            srvSocket.Bind(endPoint); // 바인딩

            byte[] recvBytes = new byte[1024]; // 바이트 받는 용량 할당
